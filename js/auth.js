...
export function renderLogin() {
    ...
    <div class="min-h-screen flex items-center justify-center p-4 bg-slate-100 animate-in fade-in duration-500">
        <div class="max-w-md w-full bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
            <div class="bg-blue-900 p-10 text-center">
                ...
                <p class="text-blue-300 text-xs mt-1 font-bold">MULTI-COMPANY CLOUD ERP</p>
            </div>
            ...
            <form id="loginForm" class="p-10 space-y-6">
                ...
                <div class="space-y-1.5">
                    <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Email do Gestor</label>
                    <input type="email" id="email" required class="w-full px-5 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:border-blue-500 outline-none transition-all" placeholder="admin@empresa.com">
                </div>

                <div class="space-y-1.5">
                    <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Palavra-passe</label>
                    <input type="password" id="password" required class="w-full px-5 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:border-blue-500 outline-none transition-all" placeholder="••••••••">
                </div>

                <button type="submit" id="loginBtn" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-xl transition-all flex items-center justify-center space-x-2">
                    <span id="btnText" class="uppercase text-xs tracking-widest">Aceder ao Painel</span>
    ...
    form.onsubmit = async (e) => {
        ...
        // Simulação de login multiempresa
        setTimeout(() => {
            const mockEmpresa = { 
                id: 'emp-imatec-001', 
            ...
            // REDIRECIONAMENTO SPA (IMEDIATO)
            renderDashboardLayout(mockUser, mockEmpresa);
            
            console.log("[IMATEC] Dashboard carregado imediatamente!");
        }, 800);
    };
}
